...
#all columns are text (int and date will be added later, but no calculations are used in the app, so it may be irrelevant)
cur.execute('DROP TABLE IF EXISTS crossectional')
cur.execute('''
CREATE TABLE "crossectional" (
    Index INT,
    NPI INT,
...
with open(fname) as csv_file:
    ...
    next(csv_file)
    for row in itertools.islice(csv_reader, i, j):
        if row[1] == 2:
            continue
        Index=count
        count+=1
        ...
        #fname1 = input('Enter the taxonomy csv file name')
        #if len(fname1) < 1 : fname1= "nucc_taxonomy_210.csv"
        with open('nucc_taxonomy_210.csv') as csv_file1:
            csv_reader1 = csv.reader(csv_file1, delimiter= ',')
            next(csv_file1)
            count = 1
            for row in csv_reader1:
                if Healthcare_Provider_Taxonomy_Code_1 == row[0]:
                    Grouping=row[1]
...
cur.close()
conn.close()
#cursor closed, and connection closed